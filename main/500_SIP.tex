...
Note that we only move agents from $N_A$ to $N_B$ or from $N_C$ to $N_D$, and therefore each agent is moved at most once, and the change of membership occurs at most $O(n)$ times; as we move agents, we must re-check whether the conditions above are met or not again, which can be done in $O(n)$ time by looking up each agent's membership and preference. Overall, in $O(n^2)$ time, we end up with a partition that no longer needs change of membership. In particular, every agent in $N_B$ has exactly one friend who belongs to either $N_B$ or $N_D$, every agent in $N_A$ has no friend or has a friend who belongs 
