...
We know from the work of Darmann et al.~\cite{GASP12WINE} that $k$-IR-Invitation is solvable in polynomial time when $\alpha = \beta = 0$.
	...
	Let us construct a (directed) ``friend graph'' ($G_f$) in which nodes represent agents and edges represent (directed) friend relationship such that there is an edge from node $i$ to node $j$ if and only if $j \in F_i$. Clearly, there is a one-to-one correspondence between an invitation and a subset of nodes. Because individual rationaliy requires that all friends of every invited agent are also invited, it is clear that if there is a path from $i$ to $j$, then inviting $i$ must 
	
	the following is true: $I$ is an IR invitation of size $k$ if and only if its corresponding set of nodes in $G_f$ has no outgoing edges that lead to a node that is not in the subset of nodes (corresponding to agents in $I$). 
