...
\end{theorem} 
\begin{proof} 
	We construct an enemy graph in which nodes represent agents and a directed edge from $i$ to $j$ is created if $j \in R_i$. Each node contributes at most one edge, and therefore the resulting graph has the following property: In each (weakly) connected component, 
