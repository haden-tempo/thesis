...
Third, if there is some agent $i\in N_A$ where $F_i = \{j\}$ with $j\in N_B$ or $j\in N_D$, then we change the approval set of $i$ as if $i$ does not approve size $k$ because agent $i$ cannot be included in our solution because its sole friend cannot be included either. 
