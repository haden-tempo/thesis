...
We know from the work of Darmann et al.~\cite{GASP12WINE} that $k$-IR-Invitation is solvable in polynomial time when $\alpha = \beta = 0$.
	...
	To find such subset(s), we can use a dynamic programming algorithm which determines the following for every integer $x \in [0, k]$ and each node $v$ in $G'$: Whether there exists a subset of nodes 
