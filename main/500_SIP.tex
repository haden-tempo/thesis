...
We interpret that agent $a_i$ is willing to attend if all friends in $F_i$ attend, no one in $E_i$ attends, and the number of attendees (including $a_i$) is acceptable (i.e., it is contained in $S_i$).
\end{definition}
...
the theorem is true when $\alpha = \beta = 0$. 
...
We know from the work of Darmann et al.~\cite{GASP12WINE} that $k$-IR-Invitation is solvable in polynomial time when $\alpha = \beta = 0$.
	...
	Let us construct a (directed) ``friend graph'' ($G_f$) in which nodes represent agents and edges represent (directed) friend relationship such that there is an edge from node $i$ to node $j$ if and only if $j \in F_i$. Clearly, there is a one-to-one correspondence between an invitation and a subset of nodes. 
	Furthermore, $I$ is an IR invitation of size $k$ if and only if its corresponding set of nodes in $G_f$ has no outgoing edges that lead to a node outside the subset and all agents in $I$ must approve of size $k$. 
...
%TODO: To mention 'max' IR-invitation can be solved in P-time.
...
\section{Parameterized Complexity} \label{SIP:sec:results}
 ...
 All entries other than ``P'' imply NP-completeness.
  ...
  Note that P and NP-completess results were presented in Section xxx. %TODO
  }
