...
Let us now construct a friendship graph in which nodes correspond to agents in $N_A$ and $N_C$. For each agent $i \in N_A \cup N_C$, we create a node $v_i$, and if $F_i = \{j\}$, then we create an edge from $v_i$ to $v_j$. This edge has the meaning that if agent $i$ is invited then agent $j$ must also be invited. Because each agent has at most one friend, each node contributes at most one edge. In addition, if add a few more edges if necessary as follows: For every agent $i \in N_A$ with $F_i = \{j\}$ for some $j$, we create an edge from $v_j$ to $v_i$ (the reverse direction) if it does not already exist. This is because if $j$ is invited, then even though $j$ may not require $i$ to attend, we must invite $i$ as well because otherwise it would violate stability (because of $i$). After we add these extra edges, we 
