...
First, we consider inputs in which the sizes of largest friend sets and enemy sets is bounded above by constants; intuitively, the smaller the constants are, the easier the problems become. 
...
We show that that \SIPs can be solved in polytime only if $\alpha$ and $\beta$ are small enough, but the problems are NP-hard in general. 
