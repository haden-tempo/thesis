...
We know from the work of Darmann et al.~\cite{GASP12WINE} that $k$-IR-Invitation is solvable in polynomial time when $\alpha = \beta = 0$.
	...
	Furthermore, $I$ is an IR invitation of size $k$ if and only if its corresponding set of nodes in $G_f$ has no outgoing edges that lead to a node outside the subset and all agents in
