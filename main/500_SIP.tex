...
Let us consider a concrete example to understand the setting. Suppose that an event organizer is trying to convene an event -- for example, a fundraiser. We assume that the time and venue for the event are fixed,
...
Not surprisingly, the complexity of \SIPs depends highly on the cardinality of friend-sets and enemy-sets, as friends-and-enemies relationship introduces combinatoric complexity in the problem of finding a good solution. 
...
In this work we take a solution concept of (Nash) stability for granted, and focus on analyzing the complexity of finding stable solutions. In particular, we are interested in finding Nash-stable invitations and individually rational 
invitations. Ballester~\cite{NP_hedonic} provides a number of computational complexity results (in fact, NP-hardness results) for finding a core-stable, Nash-stable, or individually rational outcome in hedonic games and anonymous hedonic games. However, these hardness results do not imply similar hardness results for \SIP. In hedonic games, preferences of an agent enlist subsets of agents, which may contain up to $2^{n-1}$ subsets if there are $n$ agents, and therefore the input size is not a polynomial function of $n$, while the input size of \SIPs is linear in $n$. In anonymous hedonic games, each agent only cares about the size of a coalition (or the number of participants in the same activity), and the goal is to partition agents into coalitions. In \SIP, agents do have preferences over sizes as well as constraints imposed by friends and enemies, and the goal is to choose a subset of agents (but not partition them). These two are essentially different problems, and therefore complexity results of one problem do not necessarily imply the same of the other. 
...
We then investigate parameterized complexity of the problem under the same restrictions when the size of a solution is parameterized. 
...
\begin{theorem} \label{SIP:thm:easiness_IR_a1_b0}
...
\begin{theorem} \label{SIP:thm:easiness_stable_a1_b0}
...
First, if there is some agent $i\in N_B$ with $F_i = \emptyset$, we conclude that no stable invitation of size $k$ exists because agent $i$ cannot be invited but agent $i$ will block any invitation of size $k$ as the agent approves size $k+1$ but has no friends. As we move agents from other subsets to $N_B$ in this algorithm, we will check this condition again for the new agents.
...
\end{theorem} 
\begin{proof} 
	We construct an enemy graph in which nodes represent agents and a directed edge from $i$ to $j$ is created if $j \in R_i$. Each node contributes at most one edge, and therefore the resulting graph has the following property: In each (weakly) connected component, the component is either a rooted tree (possibly just a single node) or contains a cycle such that if the edges of the cycle are removed from the component, then each node on the cycle is the root of a tree in this component. In particular, each component contains at most one cycle. 

For each connected component, we determine whether it is possible to include exactly $x$ agents in our solution (where $x\in [0,k]$) such that those $x$ agents would satisfy stability conditions (provided that we choose $k-x$ agents from other components). First assume that the component is a rooted tree. For each node $v$ in this tree, we recursive solve the following problem: Whether we can choose exactly $y \in [0,k]$ agents from the subtree rooted at $v$ such that they satisfy stability conditions when we include $v$ in our solution and when we exclude $v$ in our solution. Using dynamic programming, we can recursively solve this problem by solving leaf nodes first towards the root node. If the component contains a cycle, then let us choose an arbitrary edge on the cycle, $(v, w)$. Because there is an edge between them, we cannot include both in our solution. We consider three other cases: (a) when we include neither, (b) when we include $v$ but not $w$, and (c) when we include $w$ but not $v$. Making one of the three choices effectively removes the edge $(v,w)$ from the component, making it acyclic, and now we can solve the problem on a rooted tree instead. 

After we solve the recurrence problem for each component, we run a knapsack-like dynamic programming algorithm to determine whether we can collectively choose a subset of $k$ nodes from the entire graph that satisfy stability conditions. 
\end{proof}
...
\begin{theorem} \label{SIP:thm:nphard_a1_b1}
	Both $k$-IR-Invitation and $k$-Stable-Invitation are NP-complete when $\alpha \geq 1$ and $\beta \geq 1$. 
\end{theorem} 
\begin{proof}
...
\begin{theorem} \label{SIP:thm:nphard_a2_b0}
...
It is also worth noting that finding a maximum-size individually rational invitation is solvable in poly-time (see~\cite{LEE15AAAI}), but finding an IR invitation of a given size is NP-hard as we show in Theorem~\ref{SIP:thm:nphard_a2_b0}. The 2-SAT problem also exhibits this property in that deciding whether all clauses can be satisfied can be solved in poly-time but determining whether exactly $k$ clauses can be satisfied is NP-hard. 
...
\section{Parameterized Complexity} \label{SIP:sec:results}
 ...
 All entries other than ``P'' imply NP-completeness.
  ...
  Note that P and NP-completess results were presented early in the chapter.   }
...
\section{Symmetric Social Relationship and Complexity} \label{SIP:sec:symm}
