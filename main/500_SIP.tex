...
Now consider a friendship graph in which nodes represents agents in $N_A \cup N_C$ and a directed edge from $i$ to $j$ indicates that $j\in F_i$; that is, if we include agent $i$ in our solution, then agent $j$ must also be included. Furthermore, suppose that there is some agent $i \in N_A$ and $j\in N_C$ such that $F_i = \{j\}$. Then we add an edge from $j$ to $i$ because including agent $j$ in a solution must be accompanied by including agent $i$ (or agent $i$ would block). After this step, we find all strongly connected components in the friendship graph, and now consider each connected component as a meta node whose weight is equal to the number of nodes in the component. This reduces the friendship graph to a directed acyclic graph (DAG) with weighted meta-nodes. 
