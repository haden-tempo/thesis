...
\begin{proof}
	...
	For each agent $i$ in the original instance, we create two agents $i'$ and $i''$ who are friends of each other. We define $E_{i'}$ and $E_{i''}$ by splitting the agents in $E_i$ into the two sets; if $E_i$ contains only one or no agents, then we can 
