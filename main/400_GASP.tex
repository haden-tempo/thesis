...
Recently, Darmann~\cite{DARMANN15ADT} considered a different setting of \GASPs where agents are assumed to have strict, ordinal preferences over the outcomes, whereas both this work and \cite{GASP12WINE} assume that agents are indifferent among all outcomes that they approve of. It is an interesting future problem to consider how our results in this chapter can be extended to the ordinal setting that Darmann~\cite{DARMANN15ADT} considered. 
Lastly, Lee and Shoham~\cite{LEE15AAAI} showed that incentive compatibility and solution concepts being considered in this work are not compatible in \GASP, yet we do not consider strategic agents in this chapter (we consider strategic agents in Chapter~\ref{GT:chapter}).
