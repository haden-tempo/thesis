...
First, recall the notion of a ``popular activity'' after we color agents and activities. If a popular activity (call it $a_j$) exists for any color $d\in [1,l]$ we must reject the coloring -- because we can only assign $k_d$ agents to $a_j$ out of $|N(a_j)| > k_d$ agents,
