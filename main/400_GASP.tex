...
Due to space we omit proofs, but the following theorems can be proved in a similar way we proved Theorems~\ref{thm:stable_w1_inc}, \ref{thm:stable_fpt_dec} and \ref{thm:stable_fpt_equiv}, respectively in this order, with some modifications.

\begin{theorem} \label{thm:stable_ef_gasp_w1}
$k$-Stable-EF-GASP is $W[1]$-hard even if all agents have increasing preferences for all activities.
\end{theorem}

\begin{theorem} \label{thm:stable_ef_dec_fpt}
$k$-Stable-EF-GASP is in FPT if all agents have decreasing preferences for all activities. 
\end{theorem}

\begin{theorem} \label{thm:stable_ef_equiv_fpt}
...
In this work we investigated the parameterized complexity of the Group Activity Selection Problem (\GASP) when parameterized by the size of the solution,  different solution concepts and under various restrictions on inputs. 
