...
\begin{proof}
	...
	First, suppose that $C = \{v_1, v_2, \dots, v_k\}$ is a clique of size $k$ in $G$. We can construct a stable, envy-free assignment of size $k'$ as follows: (a) For $k^2$ copies of $v_i$, we assign them to $a_i$ if $v_i\in C$ and to $\void$ otherwise, (b) for two copies of $e_{i,j}$, we assign them to $w_{i,j}$ if $v_i\in C$ and $v_j\in C$ and to $\void$ otherwise, and (c) copies of $z$ are assigned to $\void$. Note that this assignment assigns exactly $k^3 + 2\binom{k}{2} = k^3 + k(k-1) = k'$ agents to non-void activities. We know that this assignment is IR and stable (from proof of Theorem~\ref{GASP:thm:stable_w1_inc}), so we only need to show that it is also envy-free. Any uninvited agent that is a copy of $v_i$ 
