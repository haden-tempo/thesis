...
\begin{proof} % k-EF-GASP is W[1]-hard.
	...
	Let $A_d$ be the set of activities of color $d$ and $N_d$ be the set of agents of color $d$. For every $d$ and every activity $a_j \in A_d$, let $N_d(j)$ be the set of agents in $N_d$ who approve the outcome $(a_j, k_d)$. If $|N_d| < k_d$ (or $|N_d| > k_d$), then we cannot assign $k_d$ agents of color $d$ to $a_j$ without violating IR (or envy-freeness) -- we remove $a_j$ from the instance. After removal of such activities from all colors, if any $A_d$ is empty, we reject the coloring. 
