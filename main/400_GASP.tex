...
\begin{theorem}  \label{GASP:thm:ef_fpt_equiv}
...
One can also consider a different setting where agents have a strict ordering over the set of outcomes, instead of having approved outcomes that are equally preferred. Darmann~\cite{DARMANN15ADT} recently proved several easiness and hardness results under this setting, but parameterized complexity is unknown.
Lastly, it is worth noting that Woeginger~\cite{woeginger2013core} lists variants of stability problems in hedonic coalition formation and discusses open problems, some of which are motivated by the Group Activity Selection Problem. 
