...
\begin{example}[A stable invitation may not exist] \label{GT:eg:noStableSet}
	...
	Due to $a_3$, any invitation including $a_3$ is not IR. Between $a_1$ and $a_2$ only, the empty invitation is not stable due to $a_2$, $S_1 = \{a_1\}$ is not IR due to $a_1$, $S_2 = \{a_2\}$ exhibits ER due to $a_1$, and $\{a_1, a_2\}$ is not IR due to $a_2$.
\end{example}
