...
%TODO: To define them again for ASIP?
...
Let us formally define a deterministic mechanism in the context of \ASIPs with strategic agents, and define what constitutes a strategy-proof mechanism. We later discuss in Sectino~\ref{GT:sec:asip_extension} why our technical results do not change when we consider randomized mechanisms.
...
If a mechanism chooses $I_1$ given $(S_1, S_2, S_3)$, then $a_2$ can report $(2 \succ 0 \succ 3 \sim 1)$ and make $S_2$ the only non-empty IR invitation. Similarly, if a mechanism chooses $S_2$ given $(P_1, P_2, P_3)$, $a_3$ can report $(3 \succ 0 \succ 2 \sim 1)$ so as to make $S_3$ the only non-empty IR invitation. The rest of the proof is similar to that of Theorem~\ref{GT:thm:impossibility}.
%\qed
\end{proof}
