...
To understand why strategic behavior of agents can be troublesome, we investigate the Stable Invitation Problem with minor modifications: We assume that agents have no friends or enemies while agents have total preorder over sizes of invitations instead of an approval set. Assuming ordinal preferences instead of binary preferences via approval sets makes the problem more complex, and thus all of our hardness results in previous chapter are still relevant. However, we 


let us consider a small example that features two agents in the Stable Invitations Problem.
