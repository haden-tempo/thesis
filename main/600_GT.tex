...
Consider the two agents with identical preferences from the example:
	\begin{equation*}
		...
		\end{aligned}
	\end{equation*}
	Recall that $S_1 = \{a_1\}$ and $S_2 =\{a_2\}$ are the only two stable invitations provided that both agents are truthful. If $S_1$ were to be chosen by the organizer, $a_2$ would have an incentive to act strategically, by reporting $\hat{P}_2 (1 \succ 2 \succ 0)$ instead of $P_2$. 
	Given $(P_1, \hat{P}_2)$, the only stable invitation is $S_2$ ($S_1$ now exhibits ER due to $a_2$).
