...
Consider the two agents with identical preferences from the example:
	\begin{equation*}
		\begin{aligned}
				P_1: 1 \succ 0 \succ 2,~~~& P_2: 1 \succ 0 \succ 2
		\end{aligned}
	\end{equation*}
	Recall that $I_1 = \{a_1\}$ and $I_2 =\{a_2\}$ are the only two stable invitations provided that both agents are truthful. If $I_1$ were to be chosen by the organizer, $a_2$ would have an incentive to act strategically, by reporting $\hat{P}_2 (1 \succ 2 \succ 0)$ instead of $P_2$. 
	Given $(P_1, \hat{P}_2)$, the only stable invitation is $I_2$ ($I_1$ now exhibits ER due to $a_2$).
	Notice that $a_2$ strictly prefers $I_2$ over $I_1$ (because $1 \succ_2 0$) and has an incentive to misreport in this example. By symmetry, $a_1$ may act strategically if $I_2$ were to be chosen. 
\end{proof}
