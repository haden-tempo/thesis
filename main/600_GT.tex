...
\subsection{Generalization} \label{GT:sec:generalization}

Let us now generalize our impossibility results to the Group Activity Selection Problem (\GASP) and the Stable Invitations Problem (\SIP). Our impossibility results hold for both problems because \AOIPs is a special case of these two problems, respectively.
...
Note that all of our impossibility results can be naturally extended to the Group Activity Selection Problem by Darmann et al.~\cite{GASP12WINE} since \AOIPs is a sub-class of \GASPs with a single activity. In addition, even if all agents have \INC-preferences, if there are two or more activities, then we obtain a similar impossibility result. Lastly, if agents have friends or enemies but dichotomic preferences of sizes (as in the Stable Invitations Problem), then we also obtain impossibility results.
