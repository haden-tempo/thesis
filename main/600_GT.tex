...
If a mechanism chooses $\pi_1$ given $(P_1, P_2)$, then agent $2$ has an incentive to lie about her preferences to achieve $\pi_2$. By symmetry, a mechanism cannot choose $\pi_2$, either. If it chooses $\pi_3$, both agents have incentives to lie. This shows that, even if all agents have \INC-preferences for all activities, it is impossible to achieve strategy-proofness and stability of a solution at the same time.
\end{proof}
