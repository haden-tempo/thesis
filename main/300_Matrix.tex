...
There is a trivial, brute-force algorithm that can find an optimal inspection policy: Consider every inspection policy (there are $(nm)!$ of them) and for each inspection policy compute its expected number of inspections (in time $2^{nm}$ by considering all realization of $A$). Clearly, this is not a scalable algorithm as its runtime is exponential in the size of an input matrix, and the goal in this chapter is to design an efficient algorithm.
