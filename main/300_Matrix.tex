...
In this chapter, we generalize how the organizer can poll invitees, and assume that the organizer can inspect each entry of a random matrix at unit cost (by querying a certain invitee about a certain date/time option at unit cost).
...
In this chapter, we study the Probabilistic Matrix Inspection Problem in which an individual invitee is queried over a single date/time option. In practice, an organizer may want to ask the most important (and/or busiest) invitee first about a couple of date/time options, to ensure that the person is available, before asking other invitees. For instance, when a PhD student is scheduling his/her defense of dissertation in computer science at Stanford University, it is required that both the student's primary advisor and the exam chair must be physically present while other examiners may participate virtually. Given that the committee often consists of four to five busy professors, scheduling must be done weeks or months in advance. Hence 


Given a random matrix whose entries are Bernoulli random variables, the objective is to find an optimal sequence of inspections (which is a permutation of the entries of the matrix) which minimizes the expected number of inspections until, with certainty, feasibility of the matrix is determined.
...
This generalization (from column-inspections to entry-inspections) allows for more applications that are not necessarily motivated by group scheduling settings.
