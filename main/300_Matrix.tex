...
In this chapter, we study the Probabilistic Matrix Inspection Problem in which an individual invitee is queried over a single date/time option. In practice, an organizer may want to ask the most important (and/or busiest) invitee first about a couple of date/time options, to ensure that the person is available, before asking other invitees. For instance, when a doctoral graduate student is scheduling his/her university oral examination at Stanford University (which is the defense of dissertation in computer science), it is required that both the student's primary advisor and the exam chair must be physically present while other examiners may participate virtually. %TODO CITE as of some date.
