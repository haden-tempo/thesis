...
\begin{equation} \label{eqn:expCost}
	E_c[B_{m}] = \sum_{j=1}^{m} Pr(j) \cdot C(j, B_{m})  =  \sum_{j=1}^{m} Pr(j) \cdot \left(\sum_{k=1}^{j} c(k, b_k)\right)
\end{equation}%

While one can compute $Pr(j)$ in polynomial time, it is not necessary for our algorithm. Instead we present an important lemma that leads us to an efficient algorithm. The following lemma states that if $c$ is simple, then we can compute $E_c[B_{m}]$ in a recursive manner.
...
\begin{equation}
	...
	= r_{j+1} \prod_{k=2}^{j} (1 - r_{k})  \label{eqn:pr_hat_b}
\end{equation}
...
\end{eqnarray*}
The first equality holds by definition. The second equality is due to Equation~\ref{eqn:pr_hat_b} and because $\hat{b}_k = b_{k+1}$. The third equality is by changing the range of $j$ in the summation. \\

If we multiply $E_c[\hat{B}_{m-1}]$ by $(1 - r_1)\theta$, we get the following:
\begin{equation} \label{eqn:ExpCost_Bhat}
...
\end{eqnarray*}
The first equality holds by definition of $E_c[B_{m}]$. The second equality is obtained by taking $c(1, b_1)$ out from the summation (and note that $Pr(j)$ adds up to 1) first and then applying Equation~\ref{eqn:Pr_j}. The last two inequalities hold due to Equation~\ref{eqn:ExpCost_Bhat} and \ref{eqn:r_j}, respectively. 
The last expression exactly  matches Equation~\ref{eqn:recurrence} in the lemma.
\qed
\end{proof}

Using the recurrence relation in Lemma~\ref{lemma:recurrence} and the computing method in Lemma~\ref{lemma:q_t_polytime}, we can now design an efficient recursive algorithm finds the optimal B-Doodle mechanism. Our algorithm is presented in Algorithm~\ref{alg:recursive} as a recursive method $Rec(x)$. We assume that the values of $q_t$ have been computed as a pre-processing step prior to running our algorithm, using the method in Lemma~\ref{lemma:q_t_polytime}. We further assume that time slots are sorted in decreasing order of $q_t$ (i.e. $q_1 \geq q_2 \geq \dots \geq q_{s}$); therefore we simply refer to the time slot by its index (i.e. $t = 3$ refers to the third time slot in the sorted list of time slots).
...
In Table~\ref{table:DoodleSuboptimal}, we show $S^*$ for various $(n, p)$ when $f = 1$ (i.e. the convener requires everyone be available). For instance we find $S^*(6, .8) = 5$, which implies that Doodle is suboptimal for all $s \geq 5$ given that $n=6$ and $p=.8$. The smaller $S^*$ is, the less practical Doodle is for the corresponding $(n, p)$. 
...
} \label{table:DoodleEfficiency}
\end{table}

Inefficiency of Doodle is more pronounced when the convener has a lower feasibility threshold such as $f = .7$; in such case, only a fraction of agents need to be available. 
We can clearly observe the worsened inefficiency of Doodle in Table~\ref{table:DoodleEfficiency-lower-attendance}. 
Here we show $e_{D}$ values for the same set of $(n, p)$ as in Table~\ref{table:DoodleEfficiency} but with $f = .7$. 
Previously we highlighted four entries with $e_{D} < .750$ when $f = 1$ in Table~~\ref{table:DoodleEfficiency}; when $f = .7$ the number of entires with $e_{D} < .750$ doubled to eight. 
...
Also notice that $e_{D}$ is surprisingly low in the first row of Table~\ref{table:DoodleEfficiency-lower-attendance} across all columns. This shows that regardless of the number of invitees, Doodle is significantly inefficient when the invitees are highly available ($p \geq .8$) and the convener has a relaxed feasibility threshold.
\begin{table}[h]  % \small
\centering
\begin{tabular}{|c|c|c|c|c|c|c|}
	\hline
...
Entries in boldface emphasize that $e_{D} < .750$.
} \label{table:DoodleEfficiency-lower-attendance}
\end{table}
...
For the time-averse cost function (recall $c_{\beta}(j, b) = \beta^j \cdot b$), we chose $\beta = 2$ as an experimental choice. Notice that the cost increases exponentially for later iterations, which forces the convener to send out a small number of batches (as in Doodle). We ran the same set of experiments as before to measure efficient of Doodle, $e_{D}$. 

The result is summarized in Table~\ref{table:DoodleEfficiency-lower-attendance_time_averse}, which shows the same trends as we observed in previous experiments. Notice that in the first row ($p = .8$), as $n$ increases we expect $e_{D}$ to decrease, but $e_{D}$ fluctuates while decreasing in general. The fluctuation is due to the rounding of attendance requirement ($\lceil a \cdot n \rceil$). For instance when $n = 4$, $\lceil a \cdot n \rceil$ = 3 which effectively requires 75\% of attendees be available. 
\begin{table}[h]  % \small
\centering
\begin{tabular}{|c|c|c|c|c|c|c|}
	\hline
...
Entries in boldface emphasize that $e_{D} < .750$.
} \label{table:DoodleEfficiency-lower-attendance_time_averse}
\end{table}
...
The result is summarized in Table~\ref{table:DoodleEfficiency-lower-attendance_inconvenience_averse}. Notice that $e_{D}$ is not equal to $1$ even when $p = .2$ in this setting, which we did not observe with the other cost functions previously. Due to integer-rounding, we again observe some fluctuations in $e_{D}$ across columns within the same row, but the general trend is that $e_{D}$ decreases as $n$ increases.
\begin{table}[h]  % \small
\centering
\begin{tabular}{|c|c|c|c|c|c|c|}
	\hline
...
Entries in boldface emphasize that $e_{D} < .750$.
} \label{table:DoodleEfficiency-lower-attendance_inconvenience_averse}
\end{table}
...
\end{itemize}
Intuitively the first four conditions affect $q_t$ (the probability that time slot $t$ is feasible) in the same way, and if $q_t$'s are high then Doodle is more likely to be inefficient because a few time slots may be sufficient for finding a feasible one. While the last condition is independent of $q_t$, the cost function determines what is being optimized, and Doodle becomes more inefficient if $c$ favors reducing \Inconvenience\ over reducing \Time.
...
%  \end{definition}
%
% In the random matrix setting, the organizer polling availability is equivalent to inspecting random variables at a cost to learn their realizations. An iteration consisting of options is equivalent to an inspection of columns.
% \Times is equal to the number of inspections and \Inconveniences is equal to the number of columns inspected.
% From here on, we use invitees and rows (options and columns, respectively) interchangeably as they essentially represent the same entities. We also say that the organizer inspects a column (or columns) to mean that she proposes an option (or options).
%
% Let us define a class of B-Doodle mechanisms.
