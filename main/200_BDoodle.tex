...
% \section{Introduction} \label{bdoodle:sec:intro}
...
Intuitively the first four conditions affect $q_t$ (the probability that time slot $t$ is feasible) in the same way. 


, and if $q_t$'s are high then Doodle is more likely to be inefficient because a few time slots may be sufficient for finding a feasible one. While the last condition is independent of $q_t$, the cost function determines what is being optimized, and Doodle becomes more inefficient if $c$ favors reducing \Inconveniences over reducing \Time.
