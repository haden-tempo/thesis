...
If we multiply $\Exp[\hat{B}_{m-1}]$ by $(1 - r_1)\theta$, we get the following:
\begin{equation} \label{bdoodle:eqn:ExpCost_Bhat}
(1-r_1)\theta \Exp[\hat{B}_{m-1}] = \sum_{j=2}^{m} r_{j} \left(\prod_{k=1}^{j-1} (1 - r_{k})\right) \left(\sum_{k=2}^{j} c(k, b_{k+1})\right)
\end{equation}
...
Finally we can express $\Exp[B_m]$ in terms of $\Exp[\hat{B}_{m-1}]$ as follows (where $c_1 = c(1, b_1)$ for brevity):
\begin{eqnarray*} 
\Exp[B_{m}]
...
&=& c_1 + (1-r_1)\theta \Exp[\hat{B}_{m-1}] \\
&=& c(1, b_1) + \left(\prod_{t=1}^{b_1} (1-q_t) \right)\cdot \theta \cdot \Exp[\hat{B}_{m-1}] 
\end{eqnarray*}
The first equality holds by definition of $\Exp[B_{m}]$. The second equality is obtained by taking $c(1, b_1)$ out from the summation (and note that $Pr(j)$ adds up to 1) first and then applying Equation~\ref{bdoodle:eqn:Pr_j}. The last two inequalities hold due to Equation~\ref{bdoodle:eqn:ExpCost_Bhat} and \ref{bdoodle:eqn:r_j}, respectively. 
