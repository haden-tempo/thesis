...
Given an instance $(N, S, f, c, P)$, consider some B-Doodle mechanism $B_{m} = \langle b_1, b_2, \dots, b_m \rangle$ with $\sum_{k=1}^{m} b_k = |S|$. Let $Pr(j)$ be the probability that the scheduling process ends after $j$-th iteration. Let $E_c[B_{m}]$ denote the expected cost of $B_{m}$ by $E_c[B_{m}]$, which can be expressed in terms of $Pr(\cdot)$ and the cost function $c$:
