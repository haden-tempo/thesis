...
An event organizer selects a candidate set of date/time options, and asks invitees to respond with their availability. 
Given the responses, the organizer chooses an agreeable option and announces it, or she may repeat the process by proposing another set of options if no feasible option is found. 
...
To avoid incurring much inconvenience, the organizer can select only a few options,
...
Figure~\ref{fig:bdp:Pareto_scatter} illustrates this via a simple example.
...
Figure~\ref{fig:bdp:Pareto_objective}. In this example, if the overall
...
\label{fig:bdp:Pareto_objective}
\end{figure}
...
Lastly, we assume that the event organizer is given probability estimates on availability of agents, but it is not clear how one can obtain such probabilities. 
...
\begin{definition}[B-Doodle Mechanism]\label{def:bdp:BDoodleMechanism}
...
We interpret a B-Doodle mechanism $B$ as follows: the organizer sends out options in $S_1$ during the first iteration. If a feasible option is found, the process ends. Otherwise she sends out the next batch, $S_2$, and so on.
...
\begin{definition}[Cost function] \label{def:bdp:CostFunction}
...
\begin{example}[Examples of simple cost functions] \label{exmp:bdp:CostFunctions}
	...
	\begin{eqnarray*} \label{eqn:linear_cost_function}
	...
	\begin{eqnarray*} \label{eqn:time_averse_cost_function}
	...
	\begin{eqnarray*} \label{eqn:inconvenience_averse_cost_function}
...
\begin{definition}[Batched Doodle Problem] \label{def:bdp:Problem} % \label{Def:GSP}
...
\subsection{Preliminaries}
	...
	\begin{lemma} \label{lemma:bdp:q_t_polytime}
	...
	\begin{equation} \label{eqn:vt_precompute}
	...
	Lemma~\ref{lemma:bdp:q_t_polytime} allows us to describe a B-Doodle mechanism in a simpler form, along with the following theorem because an optimal B-Doodle mechanism must inspect the columns in non-increasing order of $q_t$.
	\begin{theorem} \label{thm:bdp:exchange_argument}
...
\subsection{Optimal Algorithm} \label{sec:Algorithm}
	...
	\end{eqnarray*}
	The first equality holds by definition of $\Exp_c[B_{m}]$. The second equality is obtained by taking $c(1, b_1)$ out from the summation (and note that $\Pr(j)$ adds up to 1) first and then applying Equation~\ref{eqn:Pr_j}. The last two inequalities hold due to Equation~\ref{eqn:ExpCost_Bhat} and \ref{eqn:r_j}, respectively.
	The last expression exactly  matches Equation~\ref{eqn:recurrence} in the lemma.
	\end{proof}

	Using the recurrence relation in Lemma~\ref{lemma:recurrence} and the computing method in Lemma~\ref{lemma:bdp:q_t_polytime}, we can now design an efficient recursive algorithm finds the optimal B-Doodle mechanism. Our algorithm is presented in Algorithm~\ref{alg:recursive} as a recursive method $Rec(x)$. We assume that the values of $q_t$ have been computed as a pre-processing step prior to running our algorithm, using the method in Lemma~\ref{lemma:bdp:q_t_polytime}. We further assume that options are sorted in decreasing order of $q_t$ (i.e. $q_1 \geq q_2 \geq \dots \geq q_{s}$); therefore we simply refer to the option by its index (i.e. $t = 3$ refers to the third option in the sorted list of options).
...
In Table~\ref{table:bdp:DoodleSuboptimal}, we show $S^*$ for various $(n, p)$ when $f = 1$ (i.e. the organizer requires everyone be available). For instance we find $S^*(6, .8) = 5$, which implies that Doodle is suboptimal for all $s \geq 5$ given that $n=6$ and $p=.8$. The smaller $S^*$ is, the less practical Doodle is for the corresponding $(n, p)$.
...
} \label{table:bdp:DoodleSuboptimal}
\end{table}
...
} \label{table:bdp:DoodleEfficiency}
\end{table}
...
We can clearly observe the worsened inefficiency of Doodle in Table~\ref{table:bdp:DoodleEfficiency-lower-attendance}.
Here we show $e_{D}$ values for the same set of $(n, p)$ as in Table~\ref{table:bdp:DoodleEfficiency} but with $f = .7$.
Previously we highlighted four entries with $e_{D} < .750$ when $f = 1$ in Table~~\ref{table:bdp:DoodleEfficiency}; when $f = .7$ the number of entires with $e_{D} < .750$ doubled to eight.
...
Entries in boldface emphasize that $e_{D} < .750$.
} \label{table:bdp:DoodleEfficiency-lower-attendance}
\end{table}
...
Entries in boldface emphasize that $e_{D} < .750$.
} \label{table:bdp:DoodleEfficiency-lower-attendance_time_averse}
\end{table}
...
The result is summarized in Table~\ref{table:bdp:DoodleEfficiency-lower-attendance_inconvenience_averse}. Notice that $e_{D}$ is not equal to $1$ even when $p = .2$ in this setting, which we did not observe with the other cost functions previously. Due to integer-rounding, we again observe some fluctuations in $e_{D}$ across columns within the same row, but the general trend is that $e_{D}$ decreases as $n$ increases.
\begin{table}[h]  % \small
\centering
\begin{tabular}{|c|c|c|c|c|c|c|}
	\hline
...
Entries in boldface emphasize that $e_{D} < .750$.
} \label{table:bdp:DoodleEfficiency-lower-attendance_inconvenience_averse}
\end{table}
