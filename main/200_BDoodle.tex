...
For the inconvenience-averse cost function (recall $c_{\gamma}(j, b) = \gamma^{b}$), we chose $\gamma = 1.1$ as an experimental choice. Because the cost function is an exponential function of $b$, an optimal B-Doodle must send out small-size batches even though $\gamma$ is small. This cost function optimizes \Inconveniences . We ran the same set of experiments as before to measure efficiency of Doodle, $e_{D}$. 
