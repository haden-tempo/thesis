...
By modeling the setting as a more abstract problem using random matrices, we can apply our model and algorithms to different settings, which we discuss later in Section~\ref{bdoodle:sec:discussion}.%TODO.
...
The organizer iteratively sends out a batch of options until a feasible option is found. In our setting \Times spent by the scheduling process is measured by the number of iterations and \Inconveniences caused is measured by the number of options that have been sent out by the organizer.
...
The goal is to find a B-Doodle mechanism that minimizes the expected cost of scheduling process, given an objective function of \Times and \Inconvenience.
