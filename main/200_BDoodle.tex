...
In this section we present an algorithm that finds an optimal B-Doodle mechanism that minimizes the expected cost, given an instance of \GSP. Let us first describe how one can express the expected cost of a B-Doodle mechanism. 
...
\begin{Thm} \label{thm:recursive_algo}
Algorithm~\ref{alg:recursive} runs in polynomial time, and returns an optimal B-Doodle mechanism that minimizes the expected cost, given an instance $(N, S, f, c, P)$ of \GSP\ when $c$ is $\theta$-simple.
\end{Thm}
\begin{proof}
