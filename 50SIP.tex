...
Theorem~\ref{thm:nphard} summarizes the most relevant results of Darmann et al.~\cite{GASP12WINE} and Lee and Shoham~\cite{LEE15AAAI} on complexity of \SIPs.\footnote{
...
\section{Parameterized Complexity} \label{sec:results}
...
Note that P and NP-completeness results were known prior to this work as summarized in Theorem~\ref{thm:nphard}, but all other results are original. }
 \label{tbl:summary}
 \end{table*}


In this section, we study parameterized complexity of $k$-IR-Invitation and $k$-Stable-Invitation.
Our main contributions are summarized in Table~\ref{tbl:summary}. For instance, finding an IR invitation of size $k$ is in FPT when $\alpha = 1$ and $\beta$ is a positive constant (bounded above by some function of $k$), but finding a stable invitation in the same cases is W[1]-complete. 
 

\subsection{$k$-IR-Invitation}

Recall that $k$-IR-Invitation is the problem of finding an IR invitation of size $k$.
When $\alpha + \beta > 1$, the problem is known to be NP-hard (Theorem~\ref{thm:nphard}). 
...
\begin{theorem} \label{thm:IR_invitation_W1}
...
\begin{theorem} \label{thm:IR_invitation_FPT}
...
\begin{proof}
	...
	Let $\mathcal{A}$ be some polytime algorithm that solves $k$-IR-Invitation if $\alpha \leq 1$ and $\beta = 0$ (it exists due to Theorem~\ref{thm:nphard}). We will use $\mathcal{A}$ as a sub-routine in our FPT algorithm. Consider any coloring $c$ which colors agents using two colors $\{0,1\}$; let $c(i) \in \{0,1\}$ be the color of agent $a_i$.
...
\begin{proof}[Proof Sketch]
	...
	Note that W[1]-completenes follows from Theorem~\ref{thm:IR_invitation_W1}.
\end{proof}
...
% thm:stable_W2
\begin{theorem} \label{thm:stable_W2}
...
\begin{theorem} \label{thm:stable_FPT}
	$k$-Stable-Invitation is in FPT when $\alpha = 0$ and $\beta \leq f(k)$ where $f(k)$ can be an arbitrary function of $k$.
\end{theorem}
\begin{proof}[Proof Sketch]
	The main idea is similar to that of our proof of Theorem~\ref{thm:IR_invitation_FPT}. However, finding a stable invitation is considerably more difficult, because we must take uninvited agents into account. 
...
\begin{proof}[Proof sketch]
	...
	For each node $v_i \in V$, we first create a group of $2k^2$ node-agents (call them $G_i$) such that $G_i=\{w_{i,x}: x \in [2k^2]\}$, and define $F_{w_{i,x}} = \{w_{i,x+1}\}$ (where $w_{i,2k^2+1}$ is understood as $w_{i,1}$) and $S_{w_{i,x}} = \{k'\}$.
	 ...
	 Note that W[1]-completenes follows from Theorem~\ref{thm:stable_W2}. 
\end{proof}


\begin{theorem} \label{thm:stable_W1hard_alpha2_beta0}
...
\begin{theorem} \label{thm:stable_W2hard_beta}
...
\begin{proof}[Proof sketch]
	...
	W[2]-completenes follows from Theorem~\ref{thm:stable_W2}.
\end{proof}
...
\section{Symmetric Social Relationship} \label{sec:symm}
...
All entries other than ``P'' imply NP-completeness.
 ...
 \label{tbl:summary_symmetric}
 \end{table*}

Recall the definition of ``symmetric social relationships'' from Definition~\ref{def:symmetric_social}.
Under symmetric social relationships, both $k$-IR-Invitation and $k$-Stable-Invitation admit efficient FPT algorithms when $\beta$ is bounded, as shown in Table~\ref{tbl:summary_symmetric}, although their complexity does not change when $\beta$ is unbounded (W[1]-complete and W[2]-complete, respectively).

When we compare results in Table~\ref{tbl:summary} and Table~\ref{tbl:summary_symmetric}, two interesting observations can be made.
...
Note that our classical complexity results are original (i.e., not implied by Theorem~\ref{thm:nphard}) because Lee and Shoham~\shortcite{LEE15AAAI} did not consider the special case of symmetric social relationships. 
...
\begin{theorem} \label{thm:symmetric_IR_p_npc}
...
\begin{theorem} \label{thm:symmetric_IR_FPT}
	When agents have symmetric social relationships, 
...
\begin{proof}
	...
	Similarly to the FPT algorithm given in proof of Theorem~\ref{thm:IR_invitation_FPT}, we use Color Coding to color each node in the enemy graph as $\{0,1\}$ with equal probability.
...
Lastly we show that $k$-IR-Invitations remains to be W[1]-complete, even under symmetric social relationships, when $\beta$ is not bounded. Proof of W[1]-hardness is similar to that of Theorem~\ref{thm:IR_invitation_large_beta}, and completeness follows from Theorem~\ref{thm:IR_invitation_W1}. We omit proof of Theorem~\ref{thm:symmetric_IR_W1C}.
\begin{theorem} \label{thm:symmetric_IR_W1C}
	When agents have symmetric social relationships, 
...
\begin{theorem} \label{thm:symmetric_stable_p_npc}
	When agents have symmetric social relationships, 	
...
\begin{proof}[Proof sketch]
	...
	Our reductions for $k$-IR-Invitations in proof of Theorem~\ref{thm:symmetric_IR_p_npc} show NP-hardness for $k$-Stable-Invitations as well, because agents only approve invitations of size $k$ in our reduction; it ensures that any uninvited agent would be unwilling to attend due to the size of an invitation.
\end{proof}

\begin{theorem} \label{thm:symmetric_stable_FPT}
...
Lastly we show that $k$-Stable-Invitations remains to be W[2]-complete, even under symmetric social relationships, when $\beta$ is not bounded. Proof of W[2]-hardness is similar to that of Theorem~\ref{thm:stable_W2hard_beta}, and completeness follows from Theorem~\ref{thm:stable_W2}. We omit proof of Theorem~\ref{thm:symmetric_stable_W2C}.
\begin{theorem} \label{thm:symmetric_stable_W2C}
	When agents have symmetric social relationships, 	
